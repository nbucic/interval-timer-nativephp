...
<div
    ...
    x-data="timerAudio()"
    wire:init="@if($programId && $state === 'idle') start @endif"
>

    {{-- ── Ticker: fires every 1 s when timer is running ────────────────── --}}
    <div
        x-data="{ interval: null }"
        x-init="
            $watch('$wire.state', state => {
                clearInterval(interval);
                if (['running','pause','cooldown'].includes(state)) {
                    interval = setInterval(() => $wire.tick(), 1000);
                }
            });
            // Boot on first render
            if (['running','pause','cooldown'].includes($wire.state)) {
                interval = setInterval(() => $wire.tick(), 1000);
            }
        "
        x-effect="$wire.state"
    ...
    >
        <div class="flex items-center gap-2">
            <span
                ...
                style="background: {{ $state === 'pause' ? '#6b7280' : ($state === 'cooldown' ? '#f97316' : $phaseColor) }}"
            ></span>
    ...
    <div class="flex-1 flex flex-col items-center justify-center px-6">
        ...
        {{-- Total remaining --}}
        @if(in_array($state, ['running', 'pause', 'cooldown', 'paused']) && $totalRemaining > 0)
    ...
    <div class="flex-none px-6 pb-6 space-y-3">
        ...
        <div class="flex gap-3">
            ...
            @if(! in_array($state, ['idle', 'completed']))
            <button
                wire:click="discard"
