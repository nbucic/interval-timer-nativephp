...
function beepRunner(int $duration, int $reps = 1, int $pause = 0, int $cooldown = 0, int $leadIn = 3): object
{
    ...
    $prog->beepLeadIn = BeepLeadIn::fromNumberToEnum($leadIn);
...
test('beep fires during last 3 seconds of a 10s rep (3s lead-in)', function (): void {
    $ctx = beepRunner(duration: 10, leadIn: 3);

    // 7 ticks brings remaining from 10 → 3 (the lead-in window)
    for ($i = 0; $i < 7; $i++) $ctx->runner->tick();

    expect($ctx->beeps)->toContain('countdown');
});

test('beep fires exactly 3 times during last 3 seconds of 10s rep', function (): void {
    $ctx = beepRunner(duration: 10, leadIn: 3);
...
test('beep fires 5 times during last 5 seconds with 5s lead-in', function (): void {
...
test('segment shorter than lead-in fires beep from second 1 (2s segment, 3s lead-in)', function (): void {
    $ctx = beepRunner(duration: 2, leadIn: 3);
...
test('rep_end beep fires when rep expires', function (): void {
...
test('pause_end beep fires when inter-rep pause expires', function (): void {
...
test('cooldown_end beep fires when cooldown expires', function (): void {
...
test('rep_end does not fire when pause segment expires', function (): void {
    $ctx = beepRunner(duration: 5, reps: 2, pause: 3);
    ...
    // rep_end must NOT fire a second time at end of pause.
...
test('onPauseBeep fires exactly once when user pauses', function (): void {
    Storage::fake();
...
test('onPauseBeep does not fire on resume', function (): void {
    Storage::fake();
